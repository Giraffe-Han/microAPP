# 认证中间件

<cite>
**本文引用的文件**
- [backend/middleware/auth.js](file://backend/middleware/auth.js)
- [backend/config.js](file://backend/config.js)
- [backend/routes/admin.js](file://backend/routes/admin.js)
- [backend/routes/auth.js](file://backend/routes/auth.js)
- [backend/routes/medical.js](file://backend/routes/medical.js)
- [backend/platformAuth.js](file://backend/platformAuth.js)
- [backend/middleware/error.js](file://backend/middleware/error.js)
- [backend/middleware/validation.js](file://backend/middleware/validation.js)
- [backend/index.js](file://backend/index.js)
- [frontend/h5/src/utils/http.js](file://frontend/h5/src/utils/http.js)
- [frontend/h5/src/views/admin/composables/useAuth.js](file://frontend/h5/src/views/admin/composables/useAuth.js)
- [frontend/h5/src/router/index.js](file://frontend/h5/src/router/index.js)
- [frontend/h5/src/stores/medical.js](file://frontend/h5/src/stores/medical.js)
- [backend/package.json](file://backend/package.json)
</cite>

## 更新摘要
**变更内容**
- 新增token刷新拦截器的重大改进章节，详细说明增强的401错误处理机制
- 更新前端认证拦截器实现，包括本地存储清理和用户重定向逻辑
- 扩展认证失败处理场景，涵盖刷新端点401状态的完整处理流程
- 增强错误处理和权限控制的说明，特别是token刷新失败时的用户体验

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [医疗模块路由保护](#医疗模块路由保护)
7. [依赖关系分析](#依赖关系分析)
8. [性能考虑](#性能考虑)
9. [故障排除指南](#故障排除指南)
10. [结论](#结论)
11. [附录](#附录)

## 简介
本文件系统性地阐述认证中间件的设计与实现，重点覆盖以下方面：
- JWT 令牌提取、解码与验证机制
- authRequired 中间件的强制认证逻辑、token 格式检查与错误处理
- roleRequired 中间件的角色权限验证、allowedRoles 配置与权限拒绝处理
- optionalAuth 中间件的可选认证实现与无 token 的处理策略
- 在路由中的使用示例与组合使用场景
- 认证失败的错误码与统一响应格式
- **新增** token刷新拦截器的重大改进：增强的401错误处理、本地存储清理和用户重定向逻辑
- **新增** 医疗配送模块的路由保护机制与实现细节

## 项目结构
认证相关能力主要分布在后端中间件、路由与配置模块，并由前端发起请求时自动携带 Bearer 令牌。新增的医疗模块路由保护确保所有医疗相关功能都需要有效的用户认证。前端HTTP拦截器现在具备完善的token刷新和错误处理机制。

```mermaid
graph TB
subgraph "后端"
CFG["配置模块<br/>config.js"]
MW_AUTH["认证中间件<br/>middleware/auth.js"]
MW_ERR["错误处理中间件<br/>middleware/error.js"]
ROUTE_AUTH["认证路由<br/>routes/auth.js"]
ROUTE_ADMIN["管理路由<br/>routes/admin.js"]
ROUTE_MEDICAL["医疗路由<br/>routes/medical.js"]
PLATFORM["SSO平台对接<br/>platformAuth.js"]
end
subgraph "前端"
FE_HTTP["HTTP拦截器<br/>frontend/h5/src/utils/http.js"]
FE_ROUTER["路由守卫<br/>frontend/h5/src/router/index.js"]
FE_STORE_MEDICAL["医疗状态管理<br/>frontend/h5/src/stores/medical.js"]
FE_USEAUTH["前端认证状态<br/>frontend/h5/src/views/admin/composables/useAuth.js"]
end
FE_HTTP --> |"携带Authorization头"| ROUTE_AUTH
FE_HTTP --> |"携带Authorization头"| ROUTE_ADMIN
FE_HTTP --> |"携带Authorization头"| ROUTE_MEDICAL
FE_ROUTER --> |"路由守卫"| FE_STORE_MEDICAL
ROUTE_AUTH --> MW_AUTH
ROUTE_ADMIN --> MW_AUTH
ROUTE_MEDICAL --> MW_AUTH
MW_AUTH --> CFG
ROUTE_AUTH --> PLATFORM
MW_ERR --> ROUTE_AUTH
MW_ERR --> ROUTE_ADMIN
MW_ERR --> ROUTE_MEDICAL
```

**图表来源**
- [backend/middleware/auth.js:1-168](file://backend/middleware/auth.js#L1-L168)
- [backend/config.js:1-123](file://backend/config.js#L1-L123)
- [backend/routes/auth.js:1-591](file://backend/routes/auth.js#L1-L591)
- [backend/routes/admin.js:1-514](file://backend/routes/admin.js#L1-L514)
- [backend/routes/medical.js:1-1174](file://backend/routes/medical.js#L1-L1174)
- [backend/platformAuth.js:1-177](file://backend/platformAuth.js#L1-L177)
- [backend/middleware/error.js:1-90](file://backend/middleware/error.js#L1-L90)
- [frontend/h5/src/utils/http.js:1-119](file://frontend/h5/src/utils/http.js#L1-L119)
- [frontend/h5/src/views/admin/composables/useAuth.js:1-49](file://frontend/h5/src/views/admin/composables/useAuth.js#L1-L49)
- [frontend/h5/src/router/index.js:270-331](file://frontend/h5/src/router/index.js#L270-L331)
- [frontend/h5/src/stores/medical.js:1-301](file://frontend/h5/src/stores/medical.js#L1-L301)

**章节来源**
- [backend/middleware/auth.js:1-168](file://backend/middleware/auth.js#L1-L168)
- [backend/config.js:1-123](file://backend/config.js#L1-L123)
- [backend/routes/auth.js:1-591](file://backend/routes/auth.js#L1-L591)
- [backend/routes/admin.js:1-514](file://backend/routes/admin.js#L1-L514)
- [backend/routes/medical.js:1-1174](file://backend/routes/medical.js#L1-L1174)
- [backend/platformAuth.js:1-177](file://backend/platformAuth.js#L1-L177)
- [backend/middleware/error.js:1-90](file://backend/middleware/error.js#L1-L90)
- [frontend/h5/src/utils/http.js:1-119](file://frontend/h5/src/utils/http.js#L1-L119)
- [frontend/h5/src/views/admin/composables/useAuth.js:1-49](file://frontend/h5/src/views/admin/composables/useAuth.js#L1-L49)
- [frontend/h5/src/router/index.js:270-331](file://frontend/h5/src/router/index.js#L270-L331)
- [frontend/h5/src/stores/medical.js:1-301](file://frontend/h5/src/stores/medical.js#L1-L301)

## 核心组件
- authRequired：强制认证中间件，校验 Authorization 头与 JWT 有效性，注入 req.user
- roleRequired：基于角色的权限控制中间件，支持 allowedRoles 白名单
- optionalAuth：可选认证中间件，仅在存在有效 token 时注入用户信息
- rateLimit：简单速率限制中间件（非认证核心，但与登录等接口配合）
- **新增** HTTP拦截器：增强的token刷新和错误处理机制

上述组件均位于认证中间件模块中，依赖配置模块提供的 JWT 密钥与过期时间。

**章节来源**
- [backend/middleware/auth.js:11-101](file://backend/middleware/auth.js#L11-L101)
- [backend/config.js:9-14](file://backend/config.js#L9-L14)

## 架构总览
认证流程自前端发起请求开始，经由中间件进行令牌校验与角色权限判断，最终进入业务路由处理。前端HTTP拦截器现在具备完善的token刷新机制，能够优雅处理各种认证场景，包括刷新端点返回401状态时的错误处理。

```mermaid
sequenceDiagram
participant FE as "前端客户端"
participant HTTP as "HTTP拦截器"
participant ROUTE as "业务路由"
participant AUTHMW as "认证中间件"
participant CFG as "配置模块"
FE->>HTTP : 发起请求
HTTP->>HTTP : 附加Authorization头(Bearer token)
HTTP->>ROUTE : 转发请求
ROUTE->>AUTHMW : 应用authRequired/roleRequired/optionalAuth
AUTHMW->>CFG : 读取JWT密钥与TTL
AUTHMW->>AUTHMW : 提取并校验token
alt 令牌有效
AUTHMW->>ROUTE : 注入req.user并放行
ROUTE-->>FE : 返回业务响应
else 令牌401错误
HTTP->>HTTP : 检测401错误
alt 刷新端点401
HTTP->>HTTP : 清理本地存储并重定向
HTTP-->>FE : 返回401错误
else 其他401错误
HTTP->>HTTP : 尝试刷新token
HTTP->>HTTP : 存储新token并重试原请求
HTTP-->>FE : 返回业务响应
end
end
```

**图表来源**
- [backend/middleware/auth.js:11-101](file://backend/middleware/auth.js#L11-L101)
- [backend/config.js:9-14](file://backend/config.js#L9-L14)
- [frontend/h5/src/utils/http.js:28-98](file://frontend/h5/src/utils/http.js#L28-L98)

## 详细组件分析

### JWT 令牌验证机制
- 令牌提取：从 Authorization 头中提取 Bearer token，若缺失或格式不正确则返回 401
- 令牌解码与验证：使用配置模块中的 JWT 密钥对 token 进行签名验证
- 过期处理：捕获过期错误并返回带 TOKEN_EXPIRED 错误码的 401 响应
- 无效令牌：记录日志并返回 401

```mermaid
flowchart TD
Start(["进入中间件"]) --> CheckHeader["检查Authorization头"]
CheckHeader --> HasBearer{"以Bearer开头?"}
HasBearer --> |否| Resp401["返回401: 未提供认证令牌"]
HasBearer --> |是| ExtractToken["提取token"]
ExtractToken --> Verify["jwt.verify校验签名"]
Verify --> Ok{"校验通过?"}
Ok --> |是| InjectUser["注入req.user(id, role)"]
InjectUser --> Next["调用next()放行"]
Ok --> |否| Expired{"是否TokenExpiredError?"}
Expired --> |是| Resp401Exp["返回401+TOKEN_EXPIRED"]
Expired --> |否| LogErr["记录错误日志"] --> Resp401Inv["返回401: 无效的认证令牌"]
```

**图表来源**
- [backend/middleware/auth.js:11-45](file://backend/middleware/auth.js#L11-L45)
- [backend/config.js:9-14](file://backend/config.js#L9-L14)

**章节来源**
- [backend/middleware/auth.js:11-45](file://backend/middleware/auth.js#L11-L45)
- [backend/config.js:9-14](file://backend/config.js#L9-L14)

### authRequired 强制认证中间件
- 功能：确保每个受保护路由都必须具备有效 JWT
- 令牌格式检查：要求 Authorization 头必须为 Bearer <token> 形式
- 错误处理：
  - 缺失头：401 未提供认证令牌
  - 格式错误：401 未提供认证令牌
  - 校验失败：401 无效的认证令牌
  - 过期：401 并带 TOKEN_EXPIRED 错误码
- 成功后在 req.user 中注入用户标识与角色

```mermaid
sequenceDiagram
participant C as "客户端"
participant MW as "authRequired"
participant CFG as "配置模块"
C->>MW : 请求受保护资源
MW->>MW : 校验Authorization头
MW->>CFG : 读取JWT密钥
MW->>MW : jwt.verify(token)
alt 有效
MW-->>C : 放行并注入req.user
else 无效/过期
MW-->>C : 401 + 错误信息
end
```

**图表来源**
- [backend/middleware/auth.js:11-45](file://backend/middleware/auth.js#L11-L45)
- [backend/config.js:9-14](file://backend/config.js#L9-L14)

**章节来源**
- [backend/middleware/auth.js:11-45](file://backend/middleware/auth.js#L11-L45)

### roleRequired 角色权限验证
- 功能：基于 allowedRoles 白名单进行角色校验
- 依赖：依赖前置 authRequired 已注入 req.user
- 行为：
  - 未认证：401 未认证
  - 角色不在白名单：403 权限不足
  - 通过：放行
- 典型用法：在路由级别传入允许的角色数组

```mermaid
flowchart TD
Enter(["进入roleRequired"]) --> HasUser{"req.user存在?"}
HasUser --> |否| R401["返回401: 未认证"]
HasUser --> |是| RoleIn{"角色在allowedRoles中?"}
RoleIn --> |是| Pass["放行next()"]
RoleIn --> |否| R403["返回403: 权限不足"]
```

**图表来源**
- [backend/middleware/auth.js:50-75](file://backend/middleware/auth.js#L50-L75)

**章节来源**
- [backend/middleware/auth.js:50-75](file://backend/middleware/auth.js#L50-L75)

### optionalAuth 可选认证
- 功能：仅在存在有效 token 时才注入用户信息；无 token 或 token 无效时不阻断请求
- 场景：某些公开接口需要"尽可能识别用户"，但不强制要求登录
- 行为：
  - 无 Authorization 或非 Bearer：直接 next()
  - 有 token 且有效：注入 req.user
  - 有 token 但无效：记录调试日志并继续 next()

```mermaid
sequenceDiagram
participant C as "客户端"
participant MW as "optionalAuth"
C->>MW : 请求(可选认证)
MW->>MW : 检查Authorization头
alt 无有效token
MW-->>C : 直接放行
else 有token
MW->>MW : jwt.verify(token)
alt 有效
MW-->>C : 注入req.user并放行
else 无效
MW-->>C : 记录调试日志并放行
end
end
```

**图表来源**
- [backend/middleware/auth.js:80-101](file://backend/middleware/auth.js#L80-L101)

**章节来源**
- [backend/middleware/auth.js:80-101](file://backend/middleware/auth.js#L80-L101)

### 增强的HTTP拦截器与token刷新机制

**更新** 前端HTTP拦截器现在具备完善的token刷新和错误处理机制，能够优雅处理各种认证场景。

#### 核心功能特性
- **智能401检测**：自动检测401状态码并触发相应的处理逻辑
- **刷新端点保护**：当刷新端点返回401时，立即清理本地存储并重定向用户
- **并发请求处理**：支持多个并发请求的token刷新队列管理
- **本地存储清理**：在认证失败时自动清理access token、refresh token和用户信息
- **路径感知重定向**：根据当前路径决定重定向到登录页的策略

#### 刷新端点401错误处理流程
```mermaid
flowchart TD
Req["请求发送"] --> Check401{"响应状态401?"}
Check401 --> |否| ReturnResp["正常返回响应"]
Check401 --> |是| CheckRefresh{"是否刷新端点?"}
CheckRefresh --> |是| CleanStorage["清理本地存储"]
CleanStorage --> CheckPath{"路径是否为/medical或/admin?"}
CheckPath --> |是| Redirect["重定向到/login"]
CheckPath --> |否| Reject["拒绝请求"]
CheckRefresh --> |否| CheckRetry{"是否已重试?"}
CheckRetry --> |是| HandleError["处理认证错误"]
CheckRetry --> |否| RefreshToken["执行token刷新"]
RefreshToken --> SaveToken["保存新token"]
SaveToken --> RetryReq["重试原请求"]
```

#### 并发请求队列管理
拦截器支持多个并发请求的排队处理：
- **请求排队**：当正在进行token刷新时，其他请求会被加入队列
- **批量处理**：刷新完成后，队列中的所有请求都会使用新token重新执行
- **错误传播**：如果刷新失败，队列中的所有请求都会收到相同的错误

**章节来源**
- [frontend/h5/src/utils/http.js:28-98](file://frontend/h5/src/utils/http.js#L28-L98)

### 在路由中的使用示例
- 全站强制认证：在管理路由前挂载 authRequired，使所有子路由默认需要登录
- 局部角色控制：在特定路由上叠加 roleRequired(['admin']) 实现管理员专属接口
- 组合使用：先强制认证，再按需进行角色校验
- 可选认证：在部分公开接口使用 optionalAuth，以便在登录状态下提供个性化内容
- **新增** 医疗模块认证：所有医疗相关API都需要有效的用户认证
- **新增** 前端路由保护：医疗模块和管理模块的路由守卫确保用户登录状态

```mermaid
graph LR
A["管理路由组"] --> B["authRequired(全局登录)"]
B --> C["/users(仅admin)"]
B --> D["/services/config(按角色细化)"]
E["公开接口"] --> F["optionalAuth(可选登录)"]
G["医疗模块"] --> H["authRequired(医疗认证)"]
H --> I["/medical/certification(寄件人认证)"]
H --> J["/medical/orders(配送订单)"]
H --> K["/medical/contacts(常用联系人)"]
L["前端路由保护"] --> M["/medical(医疗模块)"]
L --> N["/admin(管理模块)"]
M --> O["检查access token"]
N --> P["检查access token和用户角色"]
```

**图表来源**
- [backend/routes/admin.js:23-24](file://backend/routes/admin.js#L23-L24)
- [backend/routes/admin.js:212-212](file://backend/routes/admin.js#L212-L212)
- [backend/routes/medical.js:16-16](file://backend/routes/medical.js#L16-L16)
- [backend/middleware/auth.js:80-101](file://backend/middleware/auth.js#L80-L101)
- [frontend/h5/src/router/index.js:278-324](file://frontend/h5/src/router/index.js#L278-L324)

**章节来源**
- [backend/routes/admin.js:23-24](file://backend/routes/admin.js#L23-L24)
- [backend/routes/admin.js:212-212](file://backend/routes/admin.js#L212-L212)
- [backend/routes/medical.js:16-16](file://backend/routes/medical.js#L16-L16)
- [backend/middleware/auth.js:80-101](file://backend/middleware/auth.js#L80-L101)
- [frontend/h5/src/router/index.js:278-324](file://frontend/h5/src/router/index.js#L278-L324)

## 医疗模块路由保护

### 医疗模块概述
医疗配送模块是一个完整的业务模块，包含以下核心功能：
- **寄件人认证**：用户身份认证与资质审核
- **起降场管理**：无人机起降点的管理与查询
- **常用联系人**：用户联系人的管理
- **配送订单**：医疗物品的配送订单管理
- **评价系统**：订单完成后的评价功能

### 后端路由保护机制
医疗模块的所有API接口都采用了严格的认证和权限控制：

#### 认证要求
- **所有医疗API**：必须通过 `authRequired` 中间件验证
- **管理端功能**：额外通过 `roleRequired(['admin', 'dsl_admin'])` 验证角色
- **用户专属功能**：通过 `req.user.id` 验证数据访问权限

#### 典型路由示例
```javascript
// 认证状态查询 - 需要登录
router.get('/certification/status', authRequired, asyncHandler(async (req, res) => {
  // 仅当前用户可访问
}));

// 创建订单 - 需要通过认证且完成寄件人认证
router.post('/orders', authRequired, asyncHandler(async (req, res) => {
  // 检查寄件人认证状态
  const certs = await readMedicalCertificationsDB();
  const myCert = certs.find(c => c.user_id === req.user.id && c.status === 'approved');
  if (!myCert) {
    return res.status(403).json({ success: false, message: '您尚未通过寄件人认证，无法下单' });
  }
  // 继续订单创建逻辑...
}));

// 管理端功能 - 需要管理员权限
router.get('/orders', authRequired, roleRequired(['admin', 'dsl_admin']), asyncHandler(async (req, res) => {
  // 管理员专用功能
}));
```

### 前端路由保护机制
前端通过路由守卫实现医疗模块的访问控制：

#### 路由守卫实现
```javascript
// 医疗模块路由保护
if (to.path.startsWith('/medical')) {
  const accessToken = authStorage.getAccessToken()
  if (!accessToken) {
    showFailToast('请先登录后再使用医疗配送功能')
    next('/login')
    return
  }
}
```

#### 医疗状态管理
前端通过 Pinia store 管理医疗模块的状态：
- **认证状态**：检查用户是否已完成寄件人认证
- **订单管理**：获取用户的医疗配送订单
- **联系人管理**：管理常用的联系人信息
- **起降场查询**：获取可用的起降场列表

**章节来源**
- [backend/routes/medical.js:196-210](file://backend/routes/medical.js#L196-L210)
- [backend/routes/medical.js:491-498](file://backend/routes/medical.js#L491-L498)
- [backend/routes/medical.js:768-797](file://backend/routes/medical.js#L768-L797)
- [frontend/h5/src/router/index.js:278-324](file://frontend/h5/src/router/index.js#L278-L324)
- [frontend/h5/src/stores/medical.js:44-74](file://frontend/h5/src/stores/medical.js#L44-L74)

## 依赖关系分析
- 认证中间件依赖配置模块提供 JWT 密钥与过期时间
- 路由层通过 require 引入中间件并组合使用
- 前端通过 HTTP 拦截器统一附加 Authorization 头
- 错误处理中间件统一捕获并格式化错误响应
- **新增** 增强的HTTP拦截器提供完整的token刷新和错误处理机制
- **新增** 医疗模块依赖认证中间件确保数据安全访问

```mermaid
graph TB
AUTHMW["认证中间件(auth.js)"] --> CFG["配置(config.js)"]
ROUTE_AUTH["认证路由(auth.js)"] --> AUTHMW
ROUTE_ADMIN["管理路由(admin.js)"] --> AUTHMW
ROUTE_MEDICAL["医疗路由(medical.js)"] --> AUTHMW
FE_HTTP["前端HTTP拦截器(http.js)"] --> ROUTE_AUTH
FE_HTTP --> ROUTE_ADMIN
FE_HTTP --> ROUTE_MEDICAL
FE_ROUTER["前端路由守卫(router)"] --> FE_STORE_MEDICAL
ERR["错误处理中间件(error.js)"] --> ROUTE_AUTH
ERR --> ROUTE_ADMIN
ERR --> ROUTE_MEDICAL
```

**图表来源**
- [backend/middleware/auth.js:1-168](file://backend/middleware/auth.js#L1-L168)
- [backend/config.js:1-123](file://backend/config.js#L1-L123)
- [backend/routes/auth.js:1-591](file://backend/routes/auth.js#L1-L591)
- [backend/routes/admin.js:1-514](file://backend/routes/admin.js#L1-L514)
- [backend/routes/medical.js:1-1174](file://backend/routes/medical.js#L1-L1174)
- [backend/middleware/error.js:1-90](file://backend/middleware/error.js#L1-L90)
- [frontend/h5/src/utils/http.js:1-119](file://frontend/h5/src/utils/http.js#L1-L119)
- [frontend/h5/src/router/index.js:270-331](file://frontend/h5/src/router/index.js#L270-L331)
- [frontend/h5/src/stores/medical.js:1-301](file://frontend/h5/src/stores/medical.js#L1-L301)

**章节来源**
- [backend/middleware/auth.js:1-168](file://backend/middleware/auth.js#L1-L168)
- [backend/config.js:1-123](file://backend/config.js#L1-L123)
- [backend/routes/auth.js:1-591](file://backend/routes/auth.js#L1-L591)
- [backend/routes/admin.js:1-514](file://backend/routes/admin.js#L1-L514)
- [backend/routes/medical.js:1-1174](file://backend/routes/medical.js#L1-L1174)
- [backend/middleware/error.js:1-90](file://backend/middleware/error.js#L1-L90)
- [frontend/h5/src/utils/http.js:1-119](file://frontend/h5/src/utils/http.js#L1-L119)
- [frontend/h5/src/router/index.js:270-331](file://frontend/h5/src/router/index.js#L270-L331)
- [frontend/h5/src/stores/medical.js:1-301](file://frontend/h5/src/stores/medical.js#L1-L301)

## 性能考虑
- 令牌验证为 O(1)，开销极低
- 建议在网关或反向代理层做基础限流，避免针对登录接口的暴力尝试
- 对于高频接口，可结合 optionalAuth 降低不必要的鉴权成本
- 前端统一拦截器复用 token，减少重复请求与网络往返
- **新增** 医疗模块采用分页查询和缓存策略，提高大数据量场景下的性能表现
- **新增** HTTP拦截器的并发请求队列管理，避免重复的token刷新操作

## 故障排除指南
- 401 未提供认证令牌
  - 检查前端是否正确设置 Authorization 头
  - 确认请求头格式为 Bearer <token>
  - **新增** 医疗模块：确认用户已完成登录且token有效
- 401 令牌已过期
  - 使用刷新令牌接口刷新 access token
  - 检查服务端 JWT 过期时间配置
  - **新增** 检查HTTP拦截器的token刷新队列是否正常工作
- 401 无效的认证令牌
  - 核对 JWT 密钥一致性
  - 检查 token 是否被篡改或来自不同签发方
  - **新增** 检查刷新端点是否返回401状态
- 403 权限不足
  - 确认用户角色是否在 allowedRoles 白名单中
  - 检查路由是否正确叠加了 roleRequired 中间件
  - **新增** 医疗模块：确认用户已完成寄件人认证
- 429 请求过于频繁,请稍后重试
  - 登录/注册接口设置了速率限制，避免频繁尝试
  - 前端应避免短时间内的重复提交
- **新增** 刷新端点401错误
  - 当刷新端点返回401时，拦截器会自动清理本地存储并重定向
  - 检查refresh token的有效性和服务器端的认证状态
  - 确认用户是否已被撤销权限或账户被禁用

**章节来源**
- [backend/middleware/auth.js:14-44](file://backend/middleware/auth.js#L14-L44)
- [backend/middleware/auth.js:53-71](file://backend/middleware/auth.js#L53-L71)
- [backend/middleware/auth.js:127-131](file://backend/middleware/auth.js#L127-L131)
- [backend/middleware/error.js:31-45](file://backend/middleware/error.js#L31-L45)
- [frontend/h5/src/utils/http.js:36-45](file://frontend/h5/src/utils/http.js#L36-L45)

## 结论
本认证中间件体系以 JWT 为核心，结合强制认证、角色权限与可选认证三种策略，满足从公开到受限的多层级安全需求。通过统一的中间件与路由组合，既保证了安全性，又提供了良好的扩展性与可维护性。**新增的增强HTTP拦截器提供了完整的token刷新和错误处理机制，特别是对刷新端点401状态的优雅处理，包括本地存储清理和用户重定向逻辑，显著提升了用户体验和系统的健壮性。** **新增的医疗模块路由保护机制进一步强化了业务数据的安全性，确保只有经过认证的用户才能访问医疗相关的功能和服务。** 建议在生产环境中严格管理 JWT 密钥与过期策略，并结合前端拦截器与错误处理机制，提供一致的用户体验。

## 附录

### 认证失败错误码与响应格式
- 401 未提供认证令牌
  - message: 未提供认证令牌
- 401 令牌已过期
  - message: 令牌已过期
  - code: TOKEN_EXPIRED
- 401 无效的认证令牌
  - message: 无效的认证令牌
- 403 权限不足
  - message: 权限不足
- 429 请求过于频繁,请稍后重试
  - message: 请求过于频繁,请稍后重试

**章节来源**
- [backend/middleware/auth.js:15-44](file://backend/middleware/auth.js#L15-L44)
- [backend/middleware/auth.js:67-71](file://backend/middleware/auth.js#L67-L71)
- [backend/middleware/auth.js:127-131](file://backend/middleware/auth.js#L127-L131)
- [backend/middleware/error.js:39-44](file://backend/middleware/error.js#L39-L44)

### 前端集成要点
- 统一在 HTTP 拦截器中附加 Authorization: Bearer <token>
- 401 时触发刷新令牌流程，刷新成功后重试原请求
- 本地持久化 access/refresh token，避免每次请求重复登录
- **新增** 医疗模块：在路由守卫中检查用户登录状态和认证状态
- **新增** 刷新端点401错误处理：自动清理本地存储并重定向到登录页
- **新增** 并发请求队列管理：支持多个请求同时刷新token的场景

**章节来源**
- [frontend/h5/src/utils/http.js:19-119](file://frontend/h5/src/utils/http.js#L19-L119)
- [frontend/h5/src/router/index.js:278-324](file://frontend/h5/src/router/index.js#L278-L324)
- [frontend/h5/src/stores/medical.js:44-74](file://frontend/h5/src/stores/medical.js#L44-L74)

### 医疗模块认证流程
```mermaid
flowchart TD
User["用户访问医疗功能"] --> CheckLogin{"是否已登录?"}
CheckLogin --> |否| RedirectLogin["重定向到登录页"]
CheckLogin --> |是| CheckCert{"是否已完成寄件人认证?"}
CheckCert --> |否| RedirectCert["重定向到认证页面"]
CheckCert --> |是| Access["允许访问医疗功能"]
RedirectLogin --> Login["用户登录"]
Login --> CheckCert
RedirectCert --> Cert["用户完成认证"]
Cert --> Access
```

**图表来源**
- [frontend/h5/src/router/index.js:278-324](file://frontend/h5/src/router/index.js#L278-L324)
- [backend/routes/medical.js:491-498](file://backend/routes/medical.js#L491-L498)

### 增强HTTP拦截器处理流程
```mermaid
flowchart TD
Request["HTTP请求"] --> AddAuth["添加Authorization头"]
AddAuth --> SendReq["发送请求"]
SendReq --> CheckResp{"响应状态"}
CheckResp --> |2xx| Success["成功响应"]
CheckResp --> |401| CheckRefresh{"是否刷新端点?"}
CheckRefresh --> |是| CleanStorage["清理本地存储"]
CleanStorage --> CheckPath{"路径是否为/medical或/admin?"}
CheckPath --> |是| Redirect["重定向到/login"]
CheckPath --> |否| Reject["拒绝请求"]
CheckRefresh --> |否| CheckRetry{"是否已重试?"}
CheckRetry --> |是| HandleError["处理认证错误"]
CheckRetry --> |否| QueueReq["加入重试队列"]
QueueReq --> RefreshToken["刷新token"]
RefreshToken --> SaveToken["保存新token"]
SaveToken --> RetryAll["重试所有队列请求"]
RetryAll --> Success
```

**图表来源**
- [frontend/h5/src/utils/http.js:28-98](file://frontend/h5/src/utils/http.js#L28-L98)