...
- [miniprogram.js](file://frontend/h5/src/utils/miniprogram.js)
...
## 更新摘要
**变更内容**   
- 新增小程序webview集成支持，增强了多端适配能力
- 更新了核心入口文件以支持小程序环境初始化
- 医疗状态管理模块（medical.js）增强了状态管理能力
- 多个视图组件添加了小程序兼容性检查和回退机制
- 新增了小程序工具函数和兼容性处理
...
**更新** 本次更新重点增强了小程序webview集成支持，使应用能够在微信小程序环境中正常运行，同时保持H5端的完整功能。

## 项目结构
前端采用 H5 移动端优先的单页应用（SPA），核心目录组织如下：
- 应用入口与根组件：main.js、App.vue
- 路由系统：router/index.js
- 状态管理：stores 下的用户、应用/订单、服务配置、医疗业务 Store
- 视图页面：views 下按功能模块划分的页面
- 组件：components 下通用组件
- 工具与网络：utils/http.js、utils/miniprogram.js
...
- [medical.js:1-150](file://frontend/h5/src/stores/medical.js#L1-L150)
...
- [miniprogram.js:1-100](file://frontend/h5/src/utils/miniprogram.js#L1-L100)
...
- 状态管理：用户状态、应用/订单状态、服务配置状态、医疗业务状态四类 Store，分别负责用户信息、申请列表与导出、服务配置与展示数据、医疗相关业务数据。
- 网络层：基于 Axios 的请求拦截器与响应拦截器，自动注入 Authorization 头与刷新令牌流程。
- 小程序兼容层：提供小程序环境检测和API兼容处理。
- 布局与通用组件：底部 Tabbar 布局、悬浮返回首页按钮等。

**更新** 新增小程序webview集成支持和医疗业务状态管理模块。
...
**更新** 新增小程序webview集成层，支持在微信小程序环境中运行H5应用。
...
Utils --> Backend["后端 API"]
Stores --> Backend
MiniProgram["小程序webview<br/>miniprogram.js"] --> Views
MiniProgram --> Utils
```
...
- 小程序检测：启动时检测运行环境，加载小程序兼容层。

**更新** 新增小程序环境检测和兼容层加载机制。
...
- 小程序兼容：在小程序环境中禁用某些浏览器特定API。

**更新** 增强路由系统在小程序环境中的兼容性处理。
...
- 医疗业务状态（medical.js）：医疗订单管理、认证状态、联系人管理、地图选择等功能状态。

**更新** 新增医疗业务状态管理模块，提供完整的医疗相关功能状态管理。
...
- [medical.js:7-150](file://frontend/h5/src/stores/medical.js#L7-L150)
...
- 小程序兼容：在小程序环境中使用 wx.request 或 uni.request 替代 fetch。

**更新** 增强网络层的小程序兼容性，支持多种请求方式。

```mermaid
sequenceDiagram
participant C as "组件"
participant AX as "Axios/小程序请求"
...
- 小程序适配：在小程序环境中调整布局和交互方式。

**更新** 布局组件增强小程序环境下的适配能力。
...
- 小程序兼容：在小程序环境中禁用不支持的API，提供降级方案。

**更新** 首页增加小程序兼容性检查和回退机制。
...
- 小程序兼容：处理小程序环境下的文件选择和表单输入限制。

**更新** 申请页面增强小程序环境下的表单处理能力。
...
- 小程序兼容：处理小程序环境下的用户信息和权限管理。

**更新** 个人中心增加小程序环境下的用户信息管理。
...
- 小程序优化：在小程序环境中启用条件编译，减少不必要的代码加载。
...
- [global.css:46-80](file://frontend/h5/src/styles/global.css#L46-80)

## 结论
该前端应用以 Vue 3 为核心，结合 Pinia 实现清晰的状态管理，配合 Vant 组件库与自定义样式，构建了移动端优先的服务平台界面。路由系统通过守卫实现鉴权与标题管理，网络层具备完善的令牌刷新机制。页面围绕"服务大厅—服务列表—详情—申请"的闭环设计，兼顾易用性与扩展性。

**更新** 本次更新显著增强了小程序webview集成支持，使应用能够在微信小程序环境中正常运行，同时保持了H5端的完整功能。新增的医疗业务状态管理模块为医疗服务提供了完善的状态管理能力。建议后续持续优化首屏性能、增强错误边界与可访问性，并完善后台管理页面的权限与数据治理。
...
- 组件使用示例与最佳实践
  ...
  - 小程序兼容：使用 miniprogram.js 工具函数进行环境检测和API兼容处理，确保跨平台一致性。
  - 医疗业务：利用 medical.js Store 管理医疗相关状态，包括订单、认证、联系人等功能。

**更新** 新增小程序兼容和医疗业务相关的最佳实践指导。